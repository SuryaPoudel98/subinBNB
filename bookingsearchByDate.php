...
$check_in_date = $_GET['in'];
$check_out_date = $_GET['out'];
$searchresult = '';
if (isset($check_in_date) and !empty($check_in_date) and strlen($check_in_date) < 31) {
    // $check_in_date = strtolower($check_in_date);
    //prepare a query and send it to the server using our search string as a wildcard on surname
    $query = "SELECT * FROM room WHERE roomname NOT IN (SELECT room_name FROM booking WHERE check_in_date >='$check_in_date' AND check_out_date <= '$check_out_date')";
    // $query = "SELECT * FROM booking WHERE check_in_date ='$check_in_date' and check_out_date='$check_out_date' ORDER BY id desc";
